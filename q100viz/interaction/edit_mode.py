...
class EditMode:
    ...
    def process_event(self, event, config):
        ...
        if event.type == KEYDOWN:
            if event.key == K_PLUS:
                ...
                session.buildings.iloc[self.polygon_selector, 7] = True  # mark as selected
                ...
                self.polygon_selector = (self.polygon_selector - 1) % (len(session.buildings))
                session.buildings.iloc[self.polygon_selector, 7] = True  # mark as selected
                session.buildings.iloc[
                    (self.polygon_selector + 1) % (len(session.buildings)), 7
                ...
                self.relevant_polygons.append(
                    session.buildings.index.values[self.polygon_selector])
    ...
    def draw(self, canvas):
        ...
        text = myFont.render(
            str(session.buildings.index.values[self.polygon_selector]), True, (255, 255, 255))
        ...
        text = myFont.render(
            str(self.polygon_selector % (len(session.buildings) - 1)) + "/" +
            str(len(session.buildings)),
            True, (255, 255, 255))
