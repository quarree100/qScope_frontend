...
class InputMode:
    def __init__(self):
        ...
        self.previous_slider_handle = None
                ...
                for x, cell in enumerate(row):
                    if cell.selected:
                        ...
                        if y == len(grid.grid)-1:
                            ...
                            if self.previous_slider_handle is not self.slider_handle:
                                session.print_verbose(
                                    ("slider_handle: ", self.slider_handle))
                                self.previous_slider_handle = self.slider_handle
    ...
    def update(self):  # TODO: move this to slider?
        # update slider:
        for grid in session.grid_1, session.grid_2:
            if grid.sliders['slider0'] is not None:
