...
def year_selection(popup):
    ...
    for year in range(config['SIMULATION_FORCE_START_YEAR'], session.modes['simulation'].max_year + divisions, divisions):
        ...
        if year < (config['SIMULATION_FORCE_START_YEAR'] + num_of_years / 2):
            popup.surface.blit(
                ...
                (session.tangibles[popup.secondary_tangible].position[0] + np.cos(np.deg2rad(rot)) * 40,
                session.tangibles[popup.secondary_tangible].position[1] + np.sin(np.deg2rad(rot)) * 40)),
            )                
    ...
    pygame.draw.line(
        ...
        (session.tangibles[popup.secondary_tangible].position[0] + np.cos(np.deg2rad(180)) * 20 * 0.7,
        session.tangibles[popup.secondary_tangible].position[1] + np.sin(np.deg2rad(180)) * 20 * 0.7),
        (session.tangibles[popup.secondary_tangible].position[0] + np.cos(np.deg2rad(180)) * 40,
        session.tangibles[popup.secondary_tangible].position[1] + np.sin(np.deg2rad(180)) * 40),
        1
    )
...
def rotation_line(popup):
    ...
    pygame.draw.line(
        ...
        (session.tangibles[popup.secondary_tangible].position[0] + np.cos(np.deg2rad(session.tangibles[popup.secondary_tangible].angle)) * popup.radius * 0.4,
        session.tangibles[popup.secondary_tangible].position[1] + np.sin(np.deg2rad(session.tangibles[popup.secondary_tangible].angle)) * popup.radius * 0.4),
        3
    )
    
def handle_information(popup):
    strings = [
        str(popup.slider.human_readable_value[popup.slider.handle]),
        str(popup.slider.human_readable_handle[popup.slider.handle]),
                ]
    for displace, string in zip([-20, 40], strings):
        ...
        popup.surface.blit(text, text.get_rect(
            center=(
                ...
                session.tangibles[popup.secondary_tangible].position[1] + displace)))
